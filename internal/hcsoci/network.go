...
// LocalNetworkSetup implements the NetworkSetup interface for configuring pod
// networking. It will setup the pods network locally.
...
func (l *LocalNetworkSetup) ConfigureNetworking(ctx context.Context, namespaceID string) error {
	...
	err = l.VM.AddEndpointsToNS(ctx, namespaceID, endpoints)
	if err != nil {
		return err
	}
	return nil
...
// pod networking. It will try and communicate with an external network configuration
...
func (e *ExternalNetworkSetup) ConfigureNetworking(ctx context.Context, namespaceID string) error {
	...
	if client == nil {
		return fmt.Errorf("no ncproxy client for UVM `%s`", e.VM.ID())
	}
