...
// Checks the payload path and PATH env variable to find application name. If the path
// is relative, E.g. path\to\binary, it will be searched for in the payload path. If it
// is solely an application name then the payload path and all directories in PATH will
// be searched. Privileged containers can run anything on the host so C:\path\to\exe, D:\path\to\exe
// etc are just passed through as is.
