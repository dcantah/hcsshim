...
type Options struct {
	...
	// count. If VirtualNodeCount is also specified this will be overridden.
	DefaultNUMA bool

	// The virtual NUMA topology configuration for the UVM. This is unmarshalled
	// (if successful) into an hcsschema.Numa struct to be embedded in the
	// VirtualMachine portion of the ComputeSystem document.
	NUMATopologyJSON string
...
func verifyOptions(ctx context.Context, options interface{}) error {
	...
	case *OptionsLCOW:
		if opts.EnableDeferredCommit && !opts.AllowOvercommit {
			return errors.New("Deferred commit is not supported on physically backed VMs")
		}
...
// logical CPU count`.
func (uvm *UtilityVM) normalizeProcessorCount(ctx context.Context, requested int32, processorTopology *hcsschema.ProcessorTopology) {
...
// MemoryInMB returns the memory assigned to the UVM in MBs
func (uvm *UtilityVM) MemorySizeInMB() int32 {
	return uvm.memorySizeInMB
}

func (uvm *UtilityVM) normalizeMemorySize(ctx context.Context, requested int32) {
