...
func (uvm *UtilityVM) validateNUMATopology(hostTop *hcsschema.ProcessorTopology, guestTop *hcsschema.Numa) error {
	...
	if len(guestTop.Settings) > int(guestTop.VirtualNodeCount) {
		return fmt.Errorf("invalid NUMA topology: number of hostTops objects exceeds virtual node count")
	}

	// Loop through NUMA hostTops and validate that there are no computeless or
