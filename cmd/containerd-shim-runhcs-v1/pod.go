...
func createPod(ctx context.Context, events publisher, req *task.CreateTaskRequest, s *specs.Spec) (shimPod, error) {
	...
	if parent != nil {
		if parent.NCProxyEnabled() {
			caAddr := fmt.Sprintf(addrFmt, req.ID)
			s, ttrpcListener, err := setupComputeAgent(caAddr, parent)
			if err != nil {
				return nil, fmt.Errorf("failed to setup compute agent service: %s", err)
			}
	...
	if isWCOW && parent != nil {
		...
		if nsid != "" {
			if err := netSetup.ConfigureNetworking(ctx, nsid); err != nil {
				return nil, fmt.Errorf("failed to setup networking for pod %s: %s", req.ID, err)
			}
		}
