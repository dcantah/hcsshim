...
func createPod(ctx context.Context, events publisher, req *task.CreateTaskRequest, s *specs.Spec) (shimPod, error) {
	...
	if isWCOW && parent != nil {
		...
		lt, err := newHcsTask(ctx, events, parent, true, req, p.privileged, s)
		if err != nil {
			return nil, err
		}
		p.sandboxTask = lt
...
type pod struct {
	...
	// privileged specifies whether this pod is for WCOW privileged containers only.
	// Normal argons can't (atleast for now) be launched in the same pod.
	privileged bool
...
func (p *pod) CreateTask(ctx context.Context, req *task.CreateTaskRequest, s *specs.Spec) (_ shimTask, err error) {
	...
	if p.privileged {
		if !oci.IsPrivileged(s) {
			return nil, errors.New("cannot create a normal process isolated container if the pod sandbox is privileged: %s")
		}
	}
	...
	st, err := newHcsTask(ctx, p.events, p.host, false, req, p.privileged, s)
	if err != nil {
		return nil, err
	}
