...
func (s *grpcService) RegisterNetworkConfigAgent(ctx context.Context, req *ncproxygrpc.RegisterNetworkConfigAgentRequest) (*ncproxygrpc.RegisterNetworkConfigAgentResponse, error) {
	...
	conn, err := grpc.Dial(req.AgentAddress, grpc.WithInsecure())
	if err != nil {
		return nil, err
	}
	...
	s.m.Lock()
	defer s.m.Unlock()
	networkToConfigAgent[req.AgentAddress] = configagent.NewNetworkConfigAgentClient(conn)
